...
2. The viewBox grows by PAD_X at the sides and PAD_Y above and below, so the
   white background does not end right at the outermost boxes.
Idempotent: a second run changes nothing.
"""
import re, sys, pathlib
PAD_X = 64  # side margin
PAD_Y = 28  # top and bottom margin
LABEL_FILL = '#ECECFF'
for f in sys.argv[1:]:
    ...
    nx, ny, nw, nh = x - PAD_X, y - PAD_Y, w + 2 * PAD_X, h + 2 * PAD_Y
